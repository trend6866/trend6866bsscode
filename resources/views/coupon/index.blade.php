...
@can('Create Product Coupon')
    <div class=" text-end d-flex all-button-box justify-content-md-end justify-content-center">
        <a class="btn btn-sm btn-primary btn-icon me-2" href="{{ route('admin.coupon.export') }}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Export') }}">
                ...
                <table class="table dataTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $coupon->coupon_uses() }}</td>
                            {{-- <td>{{ $coupon->theme_id }}</td> --}}
                            <td class="text-end">
                                @can('Show Product Coupon')
                                <a class="btn btn-sm btn-info me-2" href="{{ route('admin.coupon.show', $coupon->id) }}" >
                                    <i class="ti ti-eye py-1" data-bs-toggle="tooltip" title="view"></i>
                                </a>
                                @endcan

                                @can('Edit Product Coupon')
                                <button class="btn btn-sm btn-primary me-2"
                                    data-url="{{ route('admin.coupon.edit', $coupon->id) }}"
                                ...
                                @can('Delete Product Coupon')
                                {!! Form::open(['method' => 'DELETE', 'route' => ['admin.coupon.destroy', $coupon->id], 'class' => 'd-inline']) !!}
...
{{-- <script>
    $(document).on('change', '#category_id', function(e) {
        ...
        $.ajax({
            url: '{{ route('admin.coupon.product') }}',
            method: 'POST',
            data: data,
            context: this,
