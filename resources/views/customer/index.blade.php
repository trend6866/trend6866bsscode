...
@php
    $customer_field = \App\Models\User::customer_field();
@endphp
            ...
            <div class="card">
                <div class="card-body">
                    <div class=" multi-collapse mt-2" id="multiCollapseExample1">
                        {{ Form::open(['route' => ['admin.customer.filter.data'], 'method' => 'GET', 'id' => 'frm_submit']) }}
                        <div class="d-flex align-items-center justify-content-end card-input-wrapper">
                            ...
                            <div class="col-auto float-end ms-2 mb-4">
                                ...
                                <a href="{{ route('admin.customer.index') }}" class="btn btn-sm btn-warning"
                            ...
                            <tbody id="service-filter-data">
                                @foreach ($customers as $customer)
                                    <tr class="font-style">
                                        ...
                                        <td>
                                            @if ($activityLogEntry)
                                                <a class="text-success" href="{{ route('admin.customer.timeline', $customer->id) }}">
                                                ...
                                                {{ $customer->mobile }}
                                            @else
                                              <p class="text- capitalize text-success"> {{ $customer->first_name }} {{ $customer->last_name }} </p>
                                              {{ $customer->mobile }}
                                            @endif
                                        </td>
                                        ...
                                        <td class="Action ignore">
                                            <div class="d-flex">
                                                @if ($activityLogEntry)
                                                    <a href="{{ route('admin.customer.timeline', $customer->id) }}"
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '.page-checkbox', function() {
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: "{{ route('admin.update.customer.status') }}",
                    data: {
                        'status': status,
    ...
    <script>
        $(document).ready(function() {
            ...
            function applyFilter() {
                ...
                $.ajax({
                    url: '{{route('admin.customer.filter.data')}}',
                    type: 'GET',
                    data: data,
                    context: this,
                    success: function (data) {
