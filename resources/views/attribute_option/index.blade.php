...
<div class=" text-end d-flex all-button-box justify-content-md-end justify-content-center">
    <a href="#" class="btn btn-sm btn-primary add_attribute" data-ajax-popup="true" data-size="md"
        ...
        data-url="{{ route('admin.product-attribute-option.create',$attribute->id) }}"
...
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('admin.product-attribute.index') }}">{{ __('Attribute') }}</a></li>
                                    ...
                                    <li class="d-flex align-items-center justify-content-between list-group-item" data-id="{{$option->id}}">
                                        ...
                                        <span class="float-end">
                                            <button class="btn btn-sm btn-primary me-2"
                                                data-url="{{ route('admin.product-attribute-option.edit', $option->id) }}"
                                                data-size="md" data-ajax-popup="true"
                                                data-title="{{ __('Edit Attribute') }}" >
                                                <i class="ti ti-pencil py-1" data-bs-toggle="tooltip" title="edit"></i>
                                            </button>

                                            {!! Form::open(['method' => 'DELETE', 'route' => ['admin.product-attribute-option.destroy', $option->id], 'class' => 'd-inline']) !!}
        ...
        $(function () {
            ...
            $(".sortable").sortable({
                stop: function () {
                    ...
                    $.ajax({
                        url: "{{route('admin.attribute-option')}}",
