...
<div class="lang-setting">
    ...
    <div class="text-end d-flex all-button-box justify-content-md-end justify-content-center">
        ...
        data-url="{{ route('admin.create.language') }}" data-toggle="tooltip" title="{{ __('Create Plan') }}">
        <span>{{ __('Add') }} </span><i class="ti ti-plus "></i>
        </a>
    </div>

    @if($currantLang != (!empty(env('default_language')) ? env('default_language') : 'en'))
    {!! Form::open(['method' => 'DELETE', 'route' => ['admin.lang.destroy', $currantLang], 'class' => 'd-inline']) !!}
                ...
                <div class="card-body p-3">
                    <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        ...
                        @foreach($languages as $key=> $lang)
                            <a href="{{route('admin.manage.language',[$key])}}" class="nav-link text-sm font-weight-bold @if($currantLang == $key) active @endif">
                        ...
                        <div class="tab-pane fade show active" id="lang1" role="tabpanel" aria-labelledby="home-tab4">
                            <div class="tab-content" id="myTabContent">
                                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                                    ...
                                    @php
                                        $arrChunks = array_chunk(json_decode(json_encode($arrLabel), true), 250, true);

                                @endphp

                                @foreach($arrChunks as $chunk)
                                    <form method="post" action="{{ route('admin.store.language.data', [$currantLang]) }}">
                                        @csrf
                                        <div class="row">
...
@push('custom-script')
    <script>
        ...
        $.ajax({
                type:'POST',
                url: "{{route('admin.disablelanguage')}}",
                datType: 'json',
                data:{
                    "_token": "{{ csrf_token() }}",
                    "mode":langMode,
