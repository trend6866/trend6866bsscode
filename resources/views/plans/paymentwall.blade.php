...
var brick = new Brick({
  ...
  action: '{{route("admin.plan.pay.with.paymentwall",[$data["plan_id"],$data["coupon"]])}}',
  form: {
...
brick.showPaymentForm(function(data) {
    if(data.flag == 1){
      window.location.href ='{{route("admin.error.plan.show",1)}}';
    }else{
      window.location.href ='{{route("admin.error.plan.show",2)}}';
    }
  }, function(errors) {
    if(errors.flag == 1){
      window.location.href ='{{route("admin.error.plan.show",1)}}';
