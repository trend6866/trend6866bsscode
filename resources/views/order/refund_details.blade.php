...
@section('breadcrumb')
<li class="breadcrumb-item"><a href="{{ route('admin.RefundRequest.index') }}">{{ __('Order Refund Request') }}</a></li>
...
<div class="row">
                            ...
                            <table class="table">
                                ...
                                <tfoot>
                                    <tr>
                                        <td>  </td>
                                        <td>
                                            @foreach ($RefundStatus as $refund_setting)
                                                @if ($refund_setting->name == 'Shipment amount deduct during' && $refund_setting->is_active == '1')
    ...
    <div class="text-end">
        ...
        @foreach ($RefundStatus as $status)
            @if ($status->name == 'Manage Stock' && $status->is_active == '1')
...
<script>
    ...
    $('.accept-button').click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('admin.update-refund-status') }}',
    ...
    $('.cancel-button').click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('admin.cancel.refund.status') }}',
            data: {
                _token: '{{ csrf_token() }}',
                refund_id: refundId,
    ...
    $(document).ready(function () {
        $('.check-shipp').click(function () {
            ...
            $.ajax({
                url: '{{ route('admin.updateFinalPrice', $order['id']) }}',
                method: 'POST',
                data: {
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#refund-amount', function(e) {
            ...
            $.ajax({
                url: '{{ route('admin.refund.amount', $order['id']) }}',
                method: 'POST',
                data: data,
                context: this,
                success: function(response) {
