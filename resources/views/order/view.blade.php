...
@section('action-button')
    <div class=" text-end d-flex all-button-box justify-content-md-end justify-content-center">
        ...
        <a href="{{ route('admin.shippinglabel.pdf', \Illuminate\Support\Facades\Crypt::encrypt($order['id'])) }}"
        ...
        <a href="{{ route('admin.order.receipt', \Illuminate\Support\Facades\Crypt::encrypt($order['id'])) }}"
...
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('admin.order.index') }}">{{ __('Order') }}</a></li>
        ...
        <div class="col-sm-12">
                ...
                <div class="col-xxl-7">
                    {{-- <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <h5 class="mb-0">{{ _('Items from Order') }} {{ $order['order_id'] }} </h5>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        @foreach ($order['product'] as $item)
                                            @php
                                                $variant = \App\Models\ProductStock::where('id', $item['variant_id'])->first();
                                                    ...
                                                    @if ($order['order_status'] == 1)
                                                        @if (!empty($variant->downloadable_product) != null || !empty($product->downloadable_product) != null)
                                                            <td>
                                                                ...
                                                                @endif
                                                                <button
                                                                    class="download-btn_{{ $item['product_id'] }} me-2 action-btn btn-primary btn btn-sm align-items-center"
                            ...
                            <div class="sub-total-footer">
                                <ul class="list-unstyled mt-4">
                                    <li class="d-flex justify-content-end">
                                        ...
                                        <span class="text-start ps-3"><b>{{ SetNumberFormat($order['sub_total']) }}</b></span>
                                    </li>
                                    <li class="d-flex justify-content-end">
                                    ...
                                    @if ($order['paymnet_type'] == 'POS')
                                        <li class="d-flex justify-content-end">
                                            ...
                                            <span class="text-start ps-3"><b>{{ !empty($order['coupon_price']) ? SetNumberFormat($order['coupon_price']) : SetNumberFormat(0) }}</b></span>
                                        </li>
                                    @else
                                        <li class="d-flex justify-content-end">
                                            <span>{{ __('Discount') }} :</span>
                                            <span class="text-start ps-3"><b>{{ !empty($order['coupon_info']['discount_amount']) ? SetNumberFormat($order['coupon_info']['discount_amount']) : SetNumberFormat(0) }}</b></span>
                                        </li>
                                    @endif
                                    <li class="d-flex justify-content-end">
                                        <span>{{ __('Delivered Charges') }} :</span>
                                        <span class="text-start ps-3"><b>{{ SetNumberFormat($order['delivered_charge']) }}</b></span>
                                    </li>
                                    <li class="d-flex justify-content-end">
                                        <span><b>{{ __('Grand Total') }} :</b></span>
                                        <span class="text-start ps-3"><b>{{ SetNumberFormat($order['final_price']) }}</b></span>
                                    </li>

                                    <li class="d-flex justify-content-end">
                                        <span>{{ __('Return') }} :</span>
                                        <span class="text-start ps-3"><b>{{ SetNumberFormat($order['return_price']) }}</b></span>
                                    </li>
                                    <li class="d-flex justify-content-end">
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ __('Return') }} :</td>
                                            <td><b>{{ SetNumberFormat($order['return_price']) }}</b></td>
                                        </tr>
                                        <tr>
                    ...
                    @can('Manage Order Note')
                        <div class="card  p-0">
                            ...
                            <div class="card-body">
                                            ...
                                            <div class="card-header note-bg-color  stock-mange-note">
                                                ...
                                                <div class="cart-time-wrapper">
                                                    ...
                                                    @can('Delete Order Note')
                                                        {!! Form::open([
                                                            'method' => 'DELETE',
                                                            'route' => ['admin.order-note.destroy', $note->id],
                                                            'class' => 'd-inline',
                                                        ]) !!}
                                ...
                                @endforeach
                                @can('Create Order Note')
                                    {{ Form::open(['route' => 'admin.order-note.store', 'method' => 'POST', 'id' => 'choice_form', 'enctype' => 'multipart/form-data']) }}
                                    <div class="row">
            ...
            {{-- <div class="card">
                <div class="input-group">
                    <input type="text"
                        value="http://localhost/ecommerce/main_file/order-view/eyJpdiI6Ijl2WFh1OEpaOWplY0xIZmk5c0JLanc9PSIsInZhbHVlIjoiNm5oRGtiQ2pkTWdyUnppUTZ1TDNndz09IiwibWFjIjoiY2RhZGE5NDVhNjdkZTJkNTEwNGE3ZmRjMzBkNWU4ZjM2NmM2NGNhNmM5YTllMTg5NGQzNjlhY2UwNzVmY2FlMyIsInRhZyI6IiJ9"
                        id="myInput" class="form-control d-inline-block" aria-label="Recipient's username"
                        aria-describedby="button-addon2" readonly="">
                    <div class="input-group-append">
                        <button class="btn btn-outline-primary" type="button" onclick="myFunction()"
                            id="button-addon2"><i class="far fa-copy"></i> Copy Link</button>
                    </div>
                </div>
            </div> --}}
        </div>
...
@push('custom-script')
    ...
    <script>
        ...
        $(document).on('click', '.order_status', function() {
            ...
            $.ajax({
                url: '{{ route('admin.order.status.change', $order['id']) }}',
                method: 'POST',
                data: data,
                context: this,
                success: function(data) {
        ...
        function myFunction(item, index, arr) {
            const fileLinks = document.querySelectorAll('.downloadable_product_' + item.product_id);
            const downloadButton = document.querySelector('.download-btn_' + item.product_id);
