                            ...
                            <tbody>
                                @foreach ($jsonData as $customer)
                                    <tr class="font-style">
                                        ...
                                        @if ( in_array($customer->email,$upddata))
                                            @can('Edit Woocommerce Customer')
                                            <a href="{{ route('admin.woocom_customer.edit', $customer->id) }}"  class="btn btn-sm btn-primary"
                                                data-title="{{ __('Sync Again') }}" >
                                                <i class="ti ti-refresh " data-bs-toggle="tooltip" title="Sync Again"></i>
                                            </a>
                                            @endcan
                                        @else
                                            @can('Create Woocommerce Customer')
                                            <a href="{{ route('admin.woocom_customer.show', $customer->id) }}" class="btn btn-sm btn-primary"
