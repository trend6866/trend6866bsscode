...
{{Form::model($shippingzone, array('route' => array('admin.shippingZone.update', $shippingzone->id), 'method' => 'PUT')) }}
...
<div class="row">
    ...
    @if($shippingzone->zone_name != 'Locations not covered by your other zones')
        <div class="form-group list_height_css col-md-12">
            ...
            {!! Form::label('', __('Name'), ['class' => 'form-label']) !!}
            {!! Form::select('country_id', $country_option, null, ["class"=>"form-control country_change", "required"=>true]) !!}
        </div>
        <div class="form-group list_height_css col-md-12">
            <label>{{ __('State') }}</label>
            <div class="state">
            {!! Form::select('state_id', $state_option, null, ["class"=>"form-control state_name state_chage", "placeholder"=>"Select State", "required"=>true, 'data-select' => 0]) !!}
            </div>
        </div>
    @else
    @endif
