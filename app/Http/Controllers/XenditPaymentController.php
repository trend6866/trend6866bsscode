...
class XenditPaymentController extends Controller
{
    public function PaywithXendit(Request $request)
    {
        ...
        if ($plan) {
            ...
            $params = [
                ...
                'amount' => $get_amount,
                'callback_url' =>  route('admin.plan.xendit.status'),
                'success_redirect_url' => route('admin.plan.xendit.status', $response),
                'failure_redirect_url' => route('admin.plan.index'),
            ];
    ...
    public function PaywithXendiPayment(Request $request)
    {
        $slug = !empty($request->slug) ? $request->slug : '';
        $store = Store::where('slug',$slug)->first();
        ...
        $params = [
            ...
            'callback_url' =>  route('store.payment.Xendit', $slug),
            'success_redirect_url' => route('store.payment.Xendit',[ $slug, 'request_data' => $request->all()]),
            'failure_redirect_url' => route('admin.plan.index'),
        ];
        $invoice = \Xendit\Invoice::create($params);
        Session()->put('invoice',$invoice);
