...
class PaypalPaymentController extends Controller
{
            ...
            try {
                ...
                $price     = $plan->price;
                    ...
                    if(!empty($request->coupon))
                    {
                        ...
                        return redirect()->route('admin.plan.index')->with('error', __('This coupon code is invalid or has expired.'));
                    }
                }
                //dd($price);
                $this->paymentConfig($user);
                ...
                $response = $provider->createOrder([
                    "intent" => "CAPTURE",
                    "application_context" => [
                        "return_url" => route('admin.plan.get.payment.status', [$plan->id, $price, 'coupon_id' => $coupon_id]),
                        "cancel_url" =>  route('admin.plan.get.payment.status', [$plan->id, $price, $coupon_id]),
                    ],
                    "purchase_units" => [
                        0 => [
                            "amount" => [
    ...
    function PayWithPaypal(Request $request)
    {
        ...
        $paypal_secret = \App\Models\Utility::GetValueByName('paypal_secret', $theme_id);
        ...
        // dd($request->All() , $paypal_secret , $paypal_client_id , $paypal_mode , $CURRENCY_NAME , $CURRENCY);
        ...
        try {
            ...
            $response = $provider->createOrder([
                ...
                "application_context" => [
                    "return_url" => route('store.payment.paypal', [
                        $slug, 'request_data' => $request->all()
                    ]),
                    "cancel_url" => route('store.payment.paypal', [$slug, 'request_data' => $request->all()]),
                ],
