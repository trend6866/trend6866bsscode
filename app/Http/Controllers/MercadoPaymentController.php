...
class MercadoPaymentController extends Controller
{
    ...
    public function planPayWithMercado(Request $request)
    {
        ...
        if ($plan) {
            ...
            try {
                ...
                $success_url = route('admin.plan.mercado', [$request->plan_id, 'payment_frequency=' . $request->mercado_payment_frequency, 'coupon_id=' . $coupons_id, 'flag' => 'success']);

                $failure_url = route('admin.plan.mercado', [$request->plan_id, 'flag' => 'failure']);
                $pending_url = route('admin.plan.mercado', [$request->plan_id, 'flag' => 'pending']);
    ...
    public function PayWithMercado(Request $request)
    {
        // dd($request->all());
        ...
        try
        {
            ...
            // dd($data , $request->coupon_info);
            Session::put('request_data', $request->all());

            $success_url = route(
                'admin.mercado.callback', [
                    'flag' => 'success',
                    $slug,
                ]);
