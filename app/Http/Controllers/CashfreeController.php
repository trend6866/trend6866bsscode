...
class CashfreeController extends Controller
{
            ...
            try {
                if (!empty($request->coupon)) {
                    ...
                    $data = json_encode([
                        ...
                        "order_meta" => [
                            "return_url" => route('admin.cashfreePayment.success') . '?order_id={order_id}&order_token={order_token}&plan_id=' . $plan->id . '&amount=' . $get_amount . '&coupon=' . $coupon . ''
    ...
    public function PayWithCashfree(Request $request)
    {
        ...
        $data = json_encode([
            ...
            "customer_details" => [
                "customer_id" => 'customer_' . $request['user_id'],
                "customer_name" => $other_info->firstname,
                "customer_email" => $other_info->email,
                "customer_phone" => '1234567890',
            ],
            "order_meta" => [
                "return_url" => route('cashfreePayment.success',$slug) . '?order_id={order_id}&order_token={order_token}'.'&amount=' . $cartlist_final_price . ''
            ]
        ]);
        try {
    ...
    public function Cashfreecallback(Request $request,$slug)
    {
        ...
        if ($respons->order_id && $respons->cf_payment_id != NULL) {
            ...
            if ($info->payment_status == "SUCCESS") {
                ...
                $order->user_id = !empty($requests_data['user_id']) ? $requests_data['user_id'] : 0;
            ...
            return redirect()->route('checkout', $slug)->with('error', 'Payment Failed.');
        }
