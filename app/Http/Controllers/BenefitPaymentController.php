...
class BenefitPaymentController extends Controller
{
            ...
            try {
                ...
                $userData =
                    [
                        ...
                        "redirect" => ["url" => route('admin.benefit.call_back', ['plan_id' => $plan->id, 'amount' => $get_amount, 'coupon' => $coupon])],
    ...
    public function PayWithBenefit(Request $request)
    {
        ...
        $customerData =
            [
                ...
                "source" => ["id" => "src_bh.benefit"],
                "post" => ["url" => "https://webhook.site/fd8b0712-d70a-4280-8d6f-9f14407b3bbd"],
                "redirect" => ["url" => route('benefit.callback', ['request_data' => $request->all(), 'amount' => $cartlist_final_price, 'slug' => $slug])],
    ...
    public function benefitcallback(Request $request, $slug)
    {
        $slug = !empty($request->slug) ? $request->slug : '';
        $store = Store::where('slug', $slug)->first();
        ...
        $status_code = $datas->gateway->response->code;
        // dd($status_code);
        if ($status_code == '00') {
            ...
            //remove from cart
            Cart::where('user_id', $request->user_id)->where('product_id', $product_id)->where('variant_id', $variant_id)->where('store_id', $store->id)->delete();
