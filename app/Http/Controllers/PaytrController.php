...
class PaytrController extends Controller
{
        ...
        if ($plan) {
            ...
            try {
                ...
                $post_vals = array(
                    ...
                    'merchant_ok_url' => route('admin.pay.paytr.success', $payment_success),
                    'merchant_fail_url' => route('admin.plan.index', $payment_failed),
                ...
                if ($result['status'] == 'success') {
                    ...
                    return redirect()->route('admin.plan.index')->with('error', $result['reason']);
                }
                return view('payment.pay_tr', compact('token'));
            } catch (\Throwable $th) {
                return redirect()->route('admin.plan.index')->with('error', $th->getMessage());
            }
        }
    }
    ...
    public function PayWithPayTr(Request $request)
    {
        ...
        try {
            ...
            $post_vals = array(
                ...
                'user_address' => $user_address,
                'user_phone' => $user_phone,
                'merchant_ok_url' => route('order.paytr.status',$slug,),
                'merchant_fail_url' => route('order.paytr.status',$slug),
                'timeout_limit' => $timeout_limit,
                'currency' => $currency,
                'test_mode' => $test_mode
            );
