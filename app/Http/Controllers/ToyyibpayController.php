...
class ToyyibpayController extends Controller
{
    ...
    public function toyyibpayPaymentPrepare(Request $request)
    {
        try
        {
            ...
            if ($plan)
            {
                ...
                $this->callBackUrl = route('admin.plan.toyyibpay.callback', [$plan->id, $get_amount, $coupon]);
                $this->returnUrl = route('admin.plan.toyyibpay.callback', [$plan->id, $get_amount, $coupon]);
    ...
    public function PayWithtoyyibpay(Request $request)
    {
        // dd($request->all());
        try
        {
            ...
            $this->callBackUrl = route('store.payment.toyyibpay', [ $store->slug, $cartlist_final_price]);
            $this->returnUrl = route('store.payment.toyyibpay', [ $store->slug, $cartlist_final_price]);
