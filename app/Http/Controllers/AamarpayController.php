...
class AamarpayController extends Controller
{
        ...
        if ($plan) {
            ...
            try {
                ...
                $fields = array(
                    ...
                    'success_url' => route('admin.pay.aamarpay.success', Crypt::encrypt(['response'=>'success','coupon' => $coupon, 'plan_id' => $plan->id, 'price' => $get_amount, 'order_id' => $orderID])),
                    //your success route
                    'fail_url' => route('admin.pay.aamarpay.success', Crypt::encrypt(['response'=>'failure','coupon' => $coupon, 'plan_id' => $plan->id, 'price' => $get_amount, 'order_id' => $orderID])),
                    //your fail route
                    'cancel_url' => route('admin.pay.aamarpay.success', Crypt::encrypt(['response'=>'cancel'])),
                    //your cancel url
    ...
    public function aamarpaysuccess($data, Request $request)
    {
        ...
        {
            return redirect()->route('admin.plan.index')->with('error', __('Your payment is cancel'));
        }
        else {
            return redirect()->route('admin.plan.index')->with('error', __('Your Transaction is fail please try again'));
        }

    }

    public function PayWithAamarpaypayment(Request $request)
    {
        $url = 'https://sandbox.aamarpay.com/request.php';
        ...
        try {
            ...
            $fields = array(
                ...
                'cus_phone' => '1234567890',
                //customer phone number
                'success_url' => route('pay.aamarpay.success', [$slug,'request_data' => $request->all(),'cart_data' => $cartlist
            ]),
                //your success route
                'fail_url' => route('pay.aamarpay.success', Crypt::encrypt(['response'=>'failure','slug' => $slug, 'price' => $cartlist_final_price, 'order_id' => $orderID])),
                //your fail route
    ...
    public function Aamarpaycallback(Request $request,$slug)
    {
        if(!empty($request['request_data']['register']))
        {
            if($request['request_data']['register'] == 'on'){
                $user = User::find($request['request_data']['user_id']);
                Auth::login($user);
        ...
        $taxes = $cartlist['tax_info'];
            $billing = json_decode($request->request_data['billing_info'], true);
        ...
        $order->payment_comment = !empty($request->request_data['payment_comment']) ? $request->request_data['payment_comment'] : '';
        $order->payment_type = $request->request_data['payment_type'];
        $order->payment_status = 'Paid';
        $order->delivery_id = $requests_data['method_id'] ?? 0;
        $order->delivery_comment = !empty($request->request_data['delivery_comment']) ? $request->request_data['delivery_comment'] : '';
