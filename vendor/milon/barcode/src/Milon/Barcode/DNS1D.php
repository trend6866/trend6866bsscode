...
class DNS1D {
     ...
     * @protected
     */
    protected function getBarcodeHTML($code, $type, $w = 2, $h = 100, $color = 'black', $showCode =0) {
        if (!$this->store_path) {
            $this->setStorPath(app('config')->get("barcode.store_path"));
        }
        $this->setBarcode($code, $type);
        $html = '<div style="font-size:0;position:relative;">' . "\n";
        $html = '<div style="font-size:0;position:relative;width:100%;max-width:' . ($this->barcode_array['maxw'] * $w) . 'px;height:' . ($h) . 'px;">' . "\n";
        // print bars
        $x = 0;
        foreach ($this->barcode_array['bcode'] as $k => $v) {
            $bw = round(($v['w'] * $w), 3);
        ...
        if($showCode)
            ...
            if ($v['t']) {
                $y = round(($v['p'] * $h / $this->barcode_array['maxh']), 3);
                // draw a vertical bar
                $html .= '<div style="background-color:' . $color . ';width:100%;max-width:' . $bw . 'px;height:' . $bh . 'px;position:absolute;left:' . $x . 'px;top:' . $y . 'px;">&nbsp;</div>' . "\n";
            }
            $x += $bw;
        }
    if($showCode)
