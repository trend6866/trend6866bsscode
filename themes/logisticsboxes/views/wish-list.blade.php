        ...
        @if (count($wishlists) > 0)
            @foreach ($wishlists as $wishlist)
                ...
                <tr>
                    ...
                    <td data-label="Name">
                        <a
                            href="{{ route('page.product',[$slug,$p_id]) }}">{{ !empty($wishlist->ProductData) ? $wishlist->ProductData->name : '' }}</a>
                    ...
                    <td data-label="Action">
                        {{-- <i class="ti ti-eye text-white py-1 edit_wishlist" data-id="{{ $wishlist->id }}" data-bs-toggle="tooltip" title="delete"></i> --}}
                        <i class="ti ti-trash text-white py-1 delete_wishlist" data-id="{{ $wishlist->id }}"
...
<div class="right-result-tbl text-right">
    <b>Showing {{ $wishlists->firstItem() }}</b> to {{ $wishlists->lastItem() }} of {{ $wishlists->currentPage() }}
    ({{ $wishlists->lastPage() }} Pages)
</div>
