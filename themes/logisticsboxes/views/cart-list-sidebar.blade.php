        ...
        @if (!empty($response->data->cart_total_product))
            ...
            <div class="mini-cart-item">
                ...
                <div class="mini-cart-details">
                    <p class="mini-cart-title"><a href="{{route('page.product', [$slug,hashidsencode($item->product_id)])}}">{{ $item->name }}</a></p>
                    ...
                    <div class="pvarprice d-flex align-items-center justify-content-between">
                        <div class="price">
                            <ins>{{ $item->final_price }} <span class="currency-type">{{ $currency_name }}</span></ins><del>{{ SetNumberFormat($item->total_orignal_price) }}</del>
                        </div>
            ...
            @endforeach
        @else
            <p class="emptyCart text-center" style="color: black;">{{ __('You have no items in your shopping cart.') }}</p>
        @endif
    </div>
    <div class="mini-cart-footer mini-cart-footer1">
        @if (!empty($response->data->cart_total_product))
            <div class="u-save d-flex justify-end">
                {{ __("Subtotal") }}: {{ SetNumberFormat($response->data->sub_total) }}
            </div>
            <div class="u-save d-flex justify-end">
        ...
        <a href="{{ route('page.product-list',$slug) }}" class="btn checkout-btn" style="margin-right: 197px;">
