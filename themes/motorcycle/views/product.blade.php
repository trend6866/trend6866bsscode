...
@if ($product->product_option != NULL)
    @php
        $description = json_decode($product->product_option);
        foreach ($description as $k => $values)
        {
            $valuess = json_decode(json_encode($values), true);
            foreach ($valuess['inner-list'] as $description_val)
            {
                                        ...
                                        @endphp
                                            ...
                                            <div class="inline_contant">
                                                ...
                                                @if ($attribute->type != 'collection_horizontal')
                                                    <select class="custom-select-btn variont_option radio-btn" name="varint[{{ $attribute->name }}]">
                                                ...
                                                @else
                                                    <div class="boxed">
                                                        @foreach ($value->values as $variant2)
                                                            <input type="radio" id="{{$variant2}}" name="varint[{{ $attribute->name }}]" value="{{$variant2}}" class="custom-radio-btn variont_option radio-btn"
...
<section class="best-pro-section pdp-bst-pro padding-bottom">
    <div class="offset-container offset-right">
        ...
        <section class="services-section pdp-services">
            <div class="container">
                @php
                                    ...
                                    <div class="product-card">
                                        <div class="product-card-inner">
                                            ...
                                            <span><b>{{$review->rating_no}}</b> / 5.0</span>
                                                ...
                                                <div class="price">
                                                    <ins>{{ $bestSellers->final_price }}<span
