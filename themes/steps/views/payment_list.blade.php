            ...
            @if ($payment_data->status == 'on')
                <div class="radio-group">
                    <div class="payment_type">
                        <input type="radio" id="{{ $payment_data->name }}" name="payment_type"
                        {{ $payment_key == 0 ? 'checked' : '' }} value="{{ $payment_data->name }}" class="payment_change">
                        <label for="{{ $payment_data->name }}">
                            <span>{{ $payment_data->name_string }}</span>
                            <div class="center-descrp"> {{ $payment_data->detail }} </div>
                            <div class="radio-right">
                            @if(empty($payment_data->image))
                            <img src="{{asset(Storage::url('uploads/cod.png'))}}"    alt="cod"
                                class="paymentimag{{ $payment_data->name }}">
                            @else
                                <img src="{{get_file($payment_data->image, APP_THEME())}}"   alt="cod"
                                class="paymentimag{{ $payment_data->name }}">
                            @endif
                        </div>
                    </label>
            ...
            <div class="checkbox-custom">
                ...
                <label for="agg">
                    <span>{{ __('I have read and agree to the') }} <a
                            href="{{ route('privacy_page',$slug) }}">{{ __('Terms') }} &amp;
