            ...
            <div class="row justify-content-center">
                <div class="col-md-6 col-12">
                    <div class="section-title text-center">
                        @if(!empty($order_data))
                            <h2>{{__('Your order #')}}{{$order_data->product_order_id}} {{('has been placed!')}}</h2>
                        @else
                            ...
                            <div class="order-summery-input input-wrapper">
                                ...
                                <button class="btn btn-outline-primary" type="button" onclick="copyToClipboard(this)"
                                    id="{{ env('APP_URL')  .'/'. $slug . '/order/' . \Illuminate\Support\Facades\Crypt::encrypt($order_data->id) }}"><i
                                        class="far fa-copy"></i> Copy Link</button>
                            </div>
                        @else
                            <div class="order-summery-input input-wrapper">
                                <input type="text"
                                    value="{{ env('APP_URL') .'/'. $slug . '/order/' . \Illuminate\Support\Facades\Crypt::encrypt($order_id) }}"
                                    aria-label="Recipient's username" aria-describedby="button-addon2" readonly="">
                                <button class="btn btn-outline-primary" type="button" onclick="copyToClipboard(this)"
                                    id="{{ env('APP_URL') .'/' . $slug . '/order/' . \Illuminate\Support\Facades\Crypt::encrypt($order_id) }}"><i
                                        class="far fa-copy"></i> Copy Link</button>
                            </div>
                        @endif
                        <div class="d-flex justify-content-center backbtn">
                            <a href="{{route('landing_page',$slug)}}" class="common-btn2">
