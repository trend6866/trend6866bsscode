            ...
            @foreach ($orders as $Order)
                ...
                <tr>
                    ...
                    <td class="text-end row">
                        @if($order_data['payment_status'] == 'Unpaid' && $order_data['order_status_text'] != 'Cancel' && $Order->delivered_status == 0)
                        ...
                        <button class="btn btn-sm btn-primary me-2 "
                            data-url="{{route('customer.order',[$store->slug,Crypt::encrypt($Order->id)])}}" data-size="lg"
                            data-ajax-popup="true" data-title="{{ __('View Order') }}">
                        ...
                        @if (!$showRefundButton)
                            @if($Order['delivered_status'] == '1')
                                <button class="btn btn-sm btn-primary me-2" data-url="{{route('order.refund',[$store->slug,$Order->id,'refund'=>true])}}" data-size="lg" data-ajax-popup="true" data-title="{{ __('Order Refund') }}">
    ...
    <div class="d-flex acc-back-btn-wrp align-items-center justify-content-end">
        ...
        @endphp
         <div class="d-flex acc-back-btn-wrp align-items-center justify-content-end">
            ...
            <button class="common-btn2 btn continue-btn" onclick="get_order('{{ $nextPageUrl }}')">
                {{ __('Continue') }}
                <svg xmlns="http://www.w3.org/2000/svg" width="35" height="14" viewBox="0 0 35 14" fill="none">
                    <path d="M25.0749 14L35 7L25.0805 0L29.12 6.06667H0V7.93333H29.12L25.0749 14Z"></path>
                </svg>
